...
class ArknightsReward:
    ...
    def collect_friend_credits(self):
        ...
        # 点击信用标签
        if not self.controller.tap_image(template_name="friend_credit", max_retries=3):
            logger.error("未找到信用标签")
            # 返回主界面
            self.controller.tap_image(template_name="back_button")
            return False
    ...
    def buy_from_credit_shop(self):
        ...
        # 点击一键购买按钮
        if not self.controller.tap_image(template_name="buy_all", max_retries=3):
            logger.warning("未找到一键购买按钮，可能没有可购买的物品或信用不足")
            # 返回主界面
            self.controller.tap_image(template_name="back_button")
            return True
        ...
        # 点击确认购买按钮
        self.controller.tap_image(template_name="confirm_buy")
        time.sleep(2)
