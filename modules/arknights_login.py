...
class ArknightsLogin:
    ...
    # 登录相关界面元素模板
    TEMPLATES = {
        "start_button": "login_button.png",        # 开始游戏按钮
        ...
        for attempt in range(max_retry):
            ...
            if username and password:
                ...
                if not self._auto_login():
                    logger.error(f"自动登录失败，尝试 {attempt + 1}/{max_retry}")
                    continue
    ...
    def _wait_for_login_complete(self, timeout=60):
        ...
        # 等待主界面元素出现
        found, _, _ = self.controller.wait_for_image(
    ...
    def _handle_post_login_popups(self):
        ...
        if found:
            ...
            self.controller.tap_image(template_name="collect_login_reward")
            time.sleep(1)
        
        # 等待屏幕稳定
        self.controller.wait_stable_screen()
