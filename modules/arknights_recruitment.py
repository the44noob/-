...
class ArknightsRecruitment:
    ...
    def check_recruitment_slots(self):
        ...
        # 查找所有空闲槽位
        empty_slots = self.controller.find_all_images(template_name="empty_slot")
        empty_positions = [pos for pos, _ in empty_slots]
        
        # 查找所有已完成槽位
        finished_slots = self.controller.find_all_images(template_name="finished_slot")
    ...
    def collect_finished_recruitment(self, positions):
        ...
        for pos in positions:
            ...
            if self.controller.tap(pos[0], pos[1]):
                ...
                # 点击立即招募按钮
                if self.controller.tap_image(template_name="recruit_now"):
                    # 等待动画
                    time.sleep(1)
    ...
    def start_new_recruitment(self, positions, use_expedited=False, select_tags=True, set_time=True):
        ...
        for pos in positions:
            ...
            if self.controller.tap(pos[0], pos[1]):
                ...
                # 如果需要设置时间为9小时
                if set_time:
                    if not self.controller.tap_image(template_name="nine_hours"):
                ...
                if self.controller.tap_image(template_name="start_recruit"):
                    ...
                    # 点击确认招募按钮
                    if self.controller.tap_image(template_name="confirm_recruit"):
                        started += 1
