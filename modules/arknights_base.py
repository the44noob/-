...
class ArknightsBase:
    ...
    def navigate_to_base(self):
        ...
        # 点击进入基建按钮
        if not self.controller.tap_image(template_name="enter_base", max_retries=3):
            logger.error("未找到进入基建按钮")
            return False
        ...
        found, _, _ = self.controller.find_image(template_name="collect_all")
